...
__kernel
void fdtdKernel(__global int* idGrid, __global float* modelGrid, __global float* boundaryGrid, int idxRotate, int idxSample, __global float* input, __global float* output, int inputPosition, int outputPosition, float coeffLambda, float coeffMu, float coeffMuTwo, float coeffStringMu, float coeffStringLambda)
{
    ...
    else if(idGrid[centreIdx] == 1) {
            t1x0y0 = (((2*t0x0y0)+((coeffStringLambda)*(t0x1y0-(2*t0x0y0)+t0xM1y0))-tM1x0y0)*coeffStringMu));
    }
	else {
		t1x0y0 = (((2*t0x0y0)+((coeffMu)*tM1x0y0)+(coeffLambda*(t0x0y1+t0x0yM1+t0x1y0+t0xM1y0-(4*t0x0y0))))*(coeffMuTwo)));
    }
    ...
    //If the position is an excitation...
